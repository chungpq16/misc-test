...
def main():
    st.set_page_config(
        page_title="PM Assistant - Project Management Chatbot",
        layout="wide",
    )
    ...
    st.title("🤖 PM Assistant - Project Management Chatbot")
...
Your intelligent assistant for project management queries. I can help you with:

✅ **Project Information Queries**
- Find project details, SNAT IPs, clusters, and security zones
- Search environment configurations and loadbalancer settings
- Look up plant codes and project mappings

🔥 **Firewall Clearance Automation**
- Generate firewall clearance rules automatically
- Extract source IPs (SNAT-IP and LB-IP for opbase)
- Format rules ready for submission

💬 **Natural Language Interface**
- Ask questions in plain English
- Get instant answers from Confluence and Excel data
- View detailed SQL queries and raw data when needed
"""
    )
    ...
    # Display tables (only in DEV_MODE)
    if dev_mode:
        st.subheader("📊 Data Sources (DEV_MODE=true)")
        ...
        if df_env is not None or df_lb is not None or df_excel is not None:
            ...
            st.info(f"✅ Ready! Data sources loaded: {', '.join(tables_loaded)}")
        else:
            st.warning("⚠️ No data loaded. Check configuration.")

    # Chat Interface
    st.markdown("---")
    st.subheader("💬 Ask Me Anything About Your Projects")
...
📋 *Project Information:*
- `What is the SNAT IP of project PS-FEP-IES-Q?`
- `List all projects and SNAT IPs where customer is PS`
- `Show me all LoadBalancer entries for cluster si0dcs09`
- `What is the LB IP address for namespace bd-cros-comp02-d-ias-shared?`

🔥 *Firewall Clearance:*
- `I want to create firewall clearance for SzP, use case opbase, environment P`
- `Generate firewall rules for BuE, use case opbase, environment Q`
"""
    )
    ...
    for chat in st.session_state["chat_history"]:
        ...
        with st.chat_message("assistant"):
            is_firewall = chat.get("is_firewall", False)
            
            if is_firewall:
                # For firewall clearance: show answer and table directly
                ...
                if chat["result_df"] is not None and not chat["result_df"].empty:
                    ...
                    # Add download button
                    csv_data = chat["result_df"].to_csv(index=False).encode("utf-8")
                    st.download_button(
                        label="⬇️ Download Firewall Rules as CSV",
                        data=csv_data,
                        file_name="firewall_rules.csv",
                        mime="text/csv",
                        key=f"download_{chat['question'][:20]}",  # Unique key for each button
                    )
            else:
                # For regular queries: show answer and details in expanders
                st.markdown(f"**Answer:**\n\n{chat['answer']}")
                
                # Only show SQL in DEV_MODE
                if dev_mode:
        ...
        if df_env is None and df_lb is None and df_excel is None:
            ...
            with st.chat_message("assistant"):
                try:
                    ...
                    # Check if this is a firewall clearance request (SQL starts with comment)
                    is_firewall_request = sql.startswith("-- Firewall clearance request")

                    if is_firewall_request:
                        # For firewall clearance: show answer and table directly (not in expander)
                        st.markdown(f"**Answer:**\n\n{answer}")
                        ...
                        if result_df is not None and not result_df.empty:
                            ...
                            # Add download button for firewall rules
                    ...
                    st.session_state["chat_history"].append({
                        ...
                        "is_firewall": is_firewall_request,
                    })
